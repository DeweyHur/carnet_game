...
export default function Journey({ selected, onSelect, onAlbum, mapMode, onMap }: {
  ...
  return <>
    <div className={`journey-main${mapMode ? ' is-map' : ''}${selected ? '' : ' expanded'}`}>
      <div className="journey-heading"><div><span className="eyebrow">LE CARNET DE VOYAGE / VOL. 01</span><h1>오늘은, 어디까지 걸어볼까.</h1></div><div className="view-switch"><button className={!mapMode ? 'active' : ''} onClick={() => { if (mapMode) onMap(); }}>풍경</button><button className={mapMode ? 'active' : ''} onClick={() => { if (!mapMode) onMap(); }}>지도 ↗</button></div></div>
      {!mapMode && <>
        <div className="destination-hero">
          <TravelPhoto key={photo?.id} photo={photo} priority className="hero-photo" />
          <div className="hero-shade" />
          <div className="hero-location"><span className="destination-kicker">{here ? '● 지금 머무는 도시' : s.unlocked.includes(city.region) ? '다음 여행의 미리보기' : '아직 도착하지 않은 풍경'} · {REGIONS[city.region].name}</span><h2>{city.names.fr}</h2><p>{city.names.ko} <span>{city.coord[1].toFixed(2)}° N · {city.coord[0].toFixed(2)}° E</span></p></div>
          <div className="hero-bottom"><span>{photo?.title ?? city.names.ko}<small>{photoIndex % Math.max(1, photos.length) + 1} / {photos.length} PHOTOGRAPHS</small></span><div className="hero-controls"><button aria-label="이전 풍경" onClick={() => setPhotoIndex((i) => (i - 1 + photos.length) % photos.length)}>←</button><button aria-label="다음 풍경" onClick={() => setPhotoIndex((i) => (i + 1) % photos.length)}>→</button><button className="capture" disabled={!here || collected || !photo || !!s.travelling} onClick={() => { if (photo) s.capturePhoto(photo.id); }}>{collected ? '✓ 앨범에 담았어요' : here ? '＋ 사진 담기' : '도착해서 사진 담기'}</button></div></div>
        </div>
