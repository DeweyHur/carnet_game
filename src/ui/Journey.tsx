    ...
    <div className={`journey-main${mapMode ? ' is-map' : ''}${selected ? '' : ' expanded'}`}>
      ...
      {display(!mapMode && <>
        <div className="destination-hero">
          ...
          <div className="hero-location"><span className="destination-kicker">{display(here ? t('● 지금 머무는 도시') : s.unlocked.includes(city.region) ? t('다음 여행의 미리보기') : t('아직 도착하지 않은 풍경'))} · {t(REGIONS[city.region].name)}</span><h2>{display(city.names.fr)}</h2><p>{display(city.names.ko)} <span>{display(city.coord[1].toFixed(2))}° N · {display(city.coord[0].toFixed(2))}° E</span></p></div>
          <div className="hero-bottom"><span>{display(photo?.title ?? city.names.ko)}<small>{display(photoIndex % Math.max(1, photos.length) + 1)} / {display(photos.length)} PHOTOGRAPHS</small></span><div className="hero-controls"><button aria-label={display("이전 풍경")} onClick={() => setPhotoIndex((i) => (i - 1 + photos.length) % photos.length)}>←</button><button aria-label={display("다음 풍경")} onClick={() => setPhotoIndex((i) => (i + 1) % photos.length)}>→</button><button className="capture" disabled={!here || collected || !photo || !!s.travelling} onClick={() => { if (photo) s.capturePhoto(photo.id); }}>{display(collected ? t('✓ 앨범에 담았어요') : here ? t('＋ 사진 담기') : t('도착해서 사진 담기'))}</button></div></div>
        </div>
        <div className="journey-notes"><button className="next-assignment" onClick={() => { if (s.active) s.setPaused(false); else if (next) s.startMission(next.id); }} disabled={!!s.travelling || (!s.active && !next)}><span className="note-icon">✎</span><span><small>{display(s.active ? t('이어 쓰는 여행') : t('편집부의 다음 취재'))}</small><b>{display(s.active ? t('접어둔 미션 이어하기') : next?.title ?? t('모든 취재를 마쳤어요'))}</b></span><span className="note-arrow">↗</span></button><button className="album-note" onClick={onAlbum}><span className="note-icon">▧</span><span><small>{t('나만의 여행 앨범')}</small><b>{display(s.snapshots.length)}{display(s.lang === 'en' ? ' memories' : '장의 기억')}</b></span><span className="note-arrow">→</span></button></div>
      </>)}
      {display(mapMode && (
        <button className="map-assignment" onClick={() => { if (s.active) s.setPaused(false); else if (next) s.startMission(next.id); }} disabled={!!s.travelling || (!s.active && !next)}>
          <span className="note-icon">✎</span><span><small>{display(s.active ? t('이어 쓰는 여행') : t('편집부의 다음 취재'))}</small><b>{display(s.active ? t('접어둔 미션 이어하기') : next?.title ?? t('모든 취재를 마쳤어요'))}</b></span><span className="note-arrow">↗</span>
        </button>
      ))}
    </div>
    {display(!mapMode && <section className="destination-strip" aria-label={display("도시 여행 목록")}><div className="strip-heading"><span>{t('다음 페이지의 도시들')}</span><small>{display(s.visited.length)} / {display(CITIES.length)} CITIES VISITED</small></div><div className="destination-scroll">{display(CITIES.map((c, i) => {
      ...
      return <button key={c.id} className={`destination-card${c.id === city.id ? ' chosen' : ''}`} onClick={() => { setPhotoIndex(0); onSelect(c.id); }} aria-pressed={c.id === city.id}>
        {display(picture && <img src={picture.thumb} alt="" loading="lazy" width={240} height={150} />)}<span className="postcard-number">{display(String(i + 1).padStart(2, '0'))}</span><div><b>{display(c.names.ko)}</b><small>{display(locked ? `🔒 ${t('이야기 진행으로 열기')}` : s.visited.includes(c.id) ? t('✓ 나의 발자취') : c.names.fr)}</small></div>
      </button>;
    }))}</div></section>)}
  </>;
}
