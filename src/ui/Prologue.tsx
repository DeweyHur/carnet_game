      ...
      <div className="prologue">
        ...
        <div className="goals">
          <div className="goals-head">
            <div className="k">{display(playerName)} · {t('《Carnet》 소속 작가')}</div>
            <h2>{t('당신이 할 일')}</h2>
          </div>
          <div className="goal-cards">
            <div className="goal">
              ...
              <h3>{t('걷고, 취재한다')}</h3>
              <p>{t('실제 지도 위의 도시를 직접 찾아가 장소를 보고, 사람을 만나고, 사실 카드를 모읍니다. 모든 문장에는 출처가 붙습니다 — 그게 이 잡지의 약속입니다.')}</p>
            </div>
            <div className="goal">
              <div className="n">2</div>
              <h3>{t('기사를 쓴다')}</h3>
              <p>{t('모은 카드를 골라 기사를 조립해 편집부에 송고합니다. 수집률과 정확도로 등급 C·B·A·S가 정해지고, 등급이 원고료를 정합니다.')}</p>
            </div>
            <div className="goal">
              <div className="n">3</div>
              <h3>{t('다음 도시로 간다')}</h3>
              <p>{t('원고료가 곧 여행 자금입니다. 실제 열차 시간·요금·환율·물가로 움직입니다. 한 지역을 완주할 때마다 L.의 편지가 한 통 열리고 지도가 넓어집니다.')}</p>
            </div>
          </div>
          <div className="goals-foot">
            <div className="purse">
              {t('출발 자금')} <b>{display(fmt(wallet[home], home))}</b> <small>{t('유로는 아직 한 푼도 없습니다')} · {t(CURRENCY_META[home].name)}</small>
            </div>
            <button className="btn red" onClick={() => { primeAudio(); sfx.page(); finish(); }}>{t('편집부 문을 연다 ▸')}</button>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="prologue">
      ...
      <div className="prologue-top">
        <span className="chapter">{t(b.chapter)}</span>
        <button className="skip" onClick={() => setGoals(true)}>{t('프롤로그 건너뛰기 ⏭')}</button>
      </div>
      <div className="prologue-dim" />
      <div className="vn-dock">
        <Dialogue key={i} who={b.who} text={t(b.text)} onAdvance={next} />
      </div>
    </div>
  );
}
