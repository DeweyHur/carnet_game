  ...
  return <main className="metro-page">
    ...
    <section className="metro-experience">
      <div className="metro-main-scene"><div className="metro-scene-heading"><span className="metro-kicker">{display(arrived ? 'BONJOUR, PARIS / 도착했어요' : 'À BORD / 함께 타는 지하철')}</span><button className="metro-sound" aria-pressed={sound} onClick={() => { unlockAudio(); setSound(!sound); }}>{display("안내음 ")}{display(sound ? '켜짐' : '꺼짐')}</button></div>
        ...
        <div className="metro-playback"><div><span>{display(index + 1)} / {display(stops.length)}{display(" 여행 단계")}</span><small>{display("역당 3.5초로 압축한 노선 체험 · 실시간 운행 화면 아님")}</small></div><div>{display(arrived ? <button className="metro-primary" onClick={save} disabled={saved.includes(trip.id)}>{display(saved.includes(trip.id) ? '✓ 여행 엽서에 담았어요' : '이 여행을 엽서에 담기')}</button> : transferAhead ? <button className="metro-primary" onClick={advance}>{display(`${stops[index + 1].line}호선으로 갈아타기 →`)}</button> : <button className="metro-primary" onClick={() => { if (riding) setRiding(false); else advance(); }}>{display(riding ? '잠깐 멈추기' : index === 0 ? `${stop.line}호선 타고 출발 →` : '이어서 타기 →')}</button>)}<button onClick={() => choose(trip.id)}>{display("처음부터")}</button></div></div>
