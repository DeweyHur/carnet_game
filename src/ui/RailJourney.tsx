...
export function RailJourneyView({state,dispatch,paused,onPause,saveError,onMetro}:{state:RailState;dispatch:Dispatch<RailAction>;paused:boolean;onPause:()=>void;saveError:boolean;onMetro:()=>void}) {
  ...
  return <main className="metro-page rail-page">
    ...
    <section className="rail-hero" id="rail-top"><div><p className="metro-kicker">FRANCE / CHAPTER 02</p><h1>{d(c.title)}</h1><p>{d(c.intro)}</p><div className="rail-hero-stat">08 <span>{d(c.collected)} · {stamps} / 8</span></div></div><TravelPhoto photo={photoById('nice')} priority/><span className="rail-ticket-mark">PARIS → AILLEURS<br/>UN BILLET POUR RÊVER</span></section>
    <section className="rail-destinations"><div className="rail-filters" aria-label={d(c.all)}>{(['all','water','culture','green'] as const).map(f=><button key={f} aria-pressed={filter===f} onClick={()=>setFilter(f)}>{d(c[f])}</button>)}</div><div className="rail-cards">{RAIL_TRIPS.filter(t=>filter==='all'||t.theme===filter).map(t=><button className="rail-city" key={t.id} aria-pressed={state.selected===t.id} onClick={()=>choose(t.id)}><img src={photoById(t.id)?.thumb} alt={d(t.name)} loading="lazy" width="480" height="320"/><span className="rail-city-copy"><small>{t.train}</small><b>{d(t.name)} {state.trips[t.id]?.stamped?'✳':''}</b><span>{d(t.tagline)}</span><em>{d(c.select)} →</em></span></button>)}</div></section>
    <section className="rail-experience" ref={experience} tabIndex={-1} id="rail-experience">
      <aside className="rail-ticket"><span className="metro-kicker">CARNET / RAIL PASS</span><h2>{d(trip.name)}</h2><p>{d(trip.tagline)}</p><h3>{d(c.route)}</h3><ol><li>{trip.from}</li>{trip.via&&<li className={p.phase==='transfer'?'rail-active-stop':''}>{trip.via}<small>{d(c.transfer)}</small></li>}<li>{trip.to}</li></ol><p>{trip.train}</p><small>{d(c.routeNote)}</small><a href={railSource(trip)} target="_blank" rel="noreferrer">{d(c.source)}</a>{trip.via&&<a href={railSource(trip,true)} target="_blank" rel="noreferrer">Marseille → Nice ↗</a>}<a href={trip.guide} target="_blank" rel="noreferrer">{d(c.tourism)}</a><div className="rail-punches">○ ○ ○ ○ ○ ○ ○ ○ ○ ○</div></aside>
