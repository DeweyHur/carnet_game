  ...
  return (
    <div className="intro">
      ...
      <div className="intro-card">
        <div className="lang-switch" role="group" aria-label="Language"><button className={lang === 'ko' ? 'active' : ''} onClick={() => setLang('ko')}>{display("한국어")}</button><button className={lang === 'en' ? 'active' : ''} onClick={() => setLang('en')}>English</button></div>
        ...
        <div className="intro-features"><span><b>{display(CITIES.length)}</b>{t('개의 실제 도시')}</span><span><b>{display(MISSIONS.length)}</b>{t('개의 취재 미션')}</span><span><b>∞</b>{t('나만의 시선')}</span></div>
        <label>{t('작가 이름')}<input value={name} onChange={(e) => setName(e.target.value)} placeholder={display(lang === 'en' ? 'e.g. Jamie Carnet' : '예: 김카르네')} /></label>
        <label>{t('출신국 통화 (자국 통화 — 모든 가격에 병기됩니다)')}
          <select value={home} onChange={(e) => setHome(e.target.value as Currency)}>
            {display((['KRW', 'GBP', 'CHF', 'EUR'] as Currency[]).map((c) => <option key={c} value={c}>{display(c)} · {t(CURRENCY_META[c].name)} — {display(lang === 'en' ? 'starting budget' : '시작 예산')} {display(fmt(2000 * FX_EUR[c], c))}</option>))}
          </select>
        </label>
