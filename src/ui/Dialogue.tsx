...
import { translateDisplay as display, useT } from '../i18n';
...
export default function Dialogue({ who, name, role, color, look, text, mood, fiction, onAdvance, nextLabel, actions }: Props) {
  const t = useT();
  ...
  const next = nextLabel ?? t('계속');
  const label = who === 'player' ? (playerName || t('나'))
    : who === 'guide' ? t(name ?? '안내인')
    : who === 'echo' ? t(name ?? '메아리')
    : t({ margot: '마고 뒤랑', L: 'L.', theo: '테오', narrator: '', player: '나', guide: '', echo: '' }[who]);
  ...
  return (
    <div className={`vn-box${narr ? ' narr' : ''}${onAdvance && done ? ' clickable' : ''}`} onClick={click}>
      ...
      <div className="vn-text">
        {display(!narr && <div className="vn-name">{display(label)}{(role ?? SPEAKER_ROLE[who]) && <span className="role">{t(role ?? SPEAKER_ROLE[who])}</span>}</div>)}
        <p className="vn-line">{display(shown)}<span className="caret" style={{ opacity: done ? 0 : 1 }}>▏</span></p>
        {display(who === 'echo' && done && (
          <div className="fiction">{fiction ? t('✎ 창작 대사 — 실제 기록·저작을 바탕으로 재구성한 문장입니다.') : t('❝ 기록 인용 — 실제 저작·서한·기록에 근거한 문장입니다.')}</div>
        ))}
        <div className="vn-foot" onClick={(e) => e.stopPropagation()}>
          {actions}
          {display(onAdvance && <button className="btn sm" onClick={click}>{done ? `${next} ▸` : t('전부 보기 ⏩')}</button>)}
        </div>
      </div>
