...
function eiffel(b: LB) {
  ...
  // 부딪힘: 비스듬한 다리를 따라 층층이 안쪽으로 들어가는 상자들 — 다리 바깥 면에 14 m마다 턱이 생겨 쉬어 가며 오른다.
  // (곧은 기둥이던 때는 1층→2층이 52 m 벽이라 한 번의 기력으로는 오를 수 없었다)
  for (const [sx, sy] of [[1, 1], [1, -1], [-1, 1], [-1, -1]]) {
    for (let k = 0; k < 4; k++) { // 땅 → 1층(57 m): 다리 중심 (60,60) → (25,25), 반폭 10 → 6.5
      const z0 = k * 14.25, zm = z0 + 7.125;
      const c = 60 - (35 * zm) / 57, hw = 10 - (3.5 * zm) / 57;
      b.sbox(sx * c, sy * c, hw * 2, hw * 2, z0, z0 + 14.25);
    }
    for (let k = 0; k < 4; k++) { // 1층 → 2층(115 m): (25,25) → (13,13), 반폭 6.5 → 4
      const z0 = 58 + k * 14.25, zm = z0 + 7.125;
      const c = 25 - (12 * (zm - 57)) / 58, hw = 6.5 - (2.5 * (zm - 57)) / 58;
      b.sbox(sx * c, sy * c, hw * 2, hw * 2, z0 - 0.5, z0 + 14.25);
    }
  }
  b.sbox(0, 0, 76, 76, 50, 58);
  b.sbox(0, 0, 42, 42, 108, 115);
  // 2층 위: 15단(10.8 m) — 단마다 0.7 m씩 가늘어져 발 디딜 턱, 맨 위는 전망대(13 m)
  for (let k = 0; k < 15; k++) {
    const z0 = 115 + k * 10.8, zm = z0 + 5.4;
    const hw = 13.7 - ((zm - 115) / 160) * 10.2;
    b.sbox(0, 0, hw * 2, hw * 2, z0 - 0.5, Math.min(272, z0 + 10.8));
  }
  b.sbox(0, 0, 13, 13, 272, 277); // 전망대(바깥으로 조금 나온다 — 5 m만 더)
}
...
function louvre(b: LB) {
  ...
  // 다섯 단(4.32 m) — 단마다 그 높이 가운데의 유리 폭으로 잘라, 선 자리가 유리면에서 크게 뜨거나 파묻히지 않게
  for (let k = 0; k < 5; k++) { const w = 35 * (1 - (k + 0.5) / 5); b.sbox(0, 0, w, w, 0, 4.32 * (k + 1)); }
}
